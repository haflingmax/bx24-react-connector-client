...
export const useBX24Call = (): [
  answer: any,
  callMethod: (props: Method) => Promise<void>,
  ...
  const setCallMethod: (props: Method) => Promise<void> = async ({
    ...
    setMethodToDo((methodToDo: Method[]) => {
      const findMethod =
        key === undefined
          ? methodToDo.find((item: Method) => item.name === name)
          : undefined;
      if (findMethod !== undefined) {
        ...
        return [
          ...methodToDo,
          { name, params, readAll, callBackFunction, castomSetState, key },
        ];
      }
    });
  };
  ...
  const toDoMethod = async (methods: Method[]) => {
    if (
      ...
      (state?.EXPIRES_IN !== undefined &&
        new Date(state.EXPIRES_IN).getTime() < new Date().getTime())
    ) {
      await actions.auth();
      return;
    }
  ...
  const callMethod = async (
    { name, params, readAll }: Method,
    next: number | undefined = undefined,
    key?: string
  ): Promise<AnswerMethod | undefined> => {
    const _params = {
      ...params,
      auth: state?.ACCESS_TOKEN,
      start: next !== undefined ? next : params.start,
    };
    ...
    const finishResult: AnswerMethod =
      result.status === 0 ? {} : await result.json();

    if (readAll && finishResult.next !== undefined && finishResult?.next > 0) {
      const nextResult = await callMethod(
        { name, params, readAll },
        finishResult.next
      );
      const reduceResult: AnswerMethod = {
        ...
        result:
          nextResult?.result === undefined
            ? finishResult.result
            : collapseResult(
                finishResult.result,
                nextResult?.result !== undefined ? nextResult?.result : []
              ),
        time: {
          start: finishResult.time.start,
          date_start: finishResult.time.date_start,
          date_finish:
            nextResult?.time.date_finish === undefined
              ? finishResult.time.date_finish
              : nextResult?.time.date_finish,
          finish:
            nextResult?.time.finish === undefined
              ? finishResult.time.finish
              : nextResult?.time.finish,
          duration:
            nextResult?.time.duration === undefined
              ? finishResult.time.duration
              : nextResult?.time.duration + finishResult.time.duration,
          processing:
            nextResult?.time.processing === undefined
              ? finishResult.time.processing
              : nextResult?.time.processing + finishResult.time.processing,
        },
        ...(key !== undefined ? { key } : {}),
      };
      return reduceResult;
